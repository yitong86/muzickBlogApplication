...
public class NoteController {
    ...
    public ResponseEntity<?> testRoute() {
        return new ResponseEntity<String>("note route", HttpStatus.OK);
    }
    @PostMapping("/{listenerId}")
    public ResponseEntity<?> createNote(@PathVariable Long listenerId,@RequestBody Note newNote){
        //TODO validate the listener id is on actual listener
    ...
    public ResponseEntity<List<Note>> getNotesByListener(@PathVariable Long listener){
        List<Note> notes = noteRepository.findAllByListener_id(listener);
        return new ResponseEntity<>(notes,HttpStatus.OK);
    }
